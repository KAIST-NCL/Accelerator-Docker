...
Command::Command(const string _name, bool (*_parse_fn)(int,char**,Context*), bool (*_func)(Context*)) : name(_name), parse_fn(_parse_fn), func(_func){}
...
bool help_command(Context *ctx){
    ...
    printf(" docker run --runtime acc-runtime -e ACC_VISIBLE_DEVICES={NAME | ID}\n\n\n");
    return true;
}
